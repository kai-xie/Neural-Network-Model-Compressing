...
def param_to_compact(wb, flag=-1):
    # return: num_param, codebook, wb_low_bit, idx_low_bit
    ...
    if len(nz_wb) == 0:
        return 0, np.zeros(2**wb_bits).astype(np.float32), np.array([]).astype(np.uint8), np.array([]).astype(np.uint8)
    ...
    le_16_idx = np.nonzero( nz_idx <= 2**idx_bits)[0]
    le_16_value = nz_idx[le_16_idx]
    
    gt_16_idx = np.nonzero( nz_idx > 2**idx_bits)[0]
    gt_16_value = nz_idx[gt_16_idx]

    # the number of 256 that is tobe filled before the bigger idx diff
    filler = (gt_16_value -1)/(2**idx_bits)
    ...
    gt_16_value_new = gt_16_value - filler*(2**idx_bits)
    ...
    for i, idx in enumerate(nz_idx):
        if idx <= 2**wb_bits:
            ...
            # insert some 2**wb_bits to param_idx
            num_to_insert = (idx-1)// (2**wb_bits)
            num_inserted += num_to_insert
            param_idx = np.concatenate((param_idx, np.array([2**wb_bits]*num_to_insert), np.array([idx - 2**wb_bits*num_to_insert])))
            param_idx_valued = np.append(param_idx_valued, i+num_inserted)
    ...
    # num of params to be stored, including the padding zeros.
    ...
    # set the range of param_idx: [0, 2**idx_bits-1]
    param_idx = param_idx - 1
    if (param_idx >= 2**idx_bits).any():
        print "Error: param_idx should not be great than or equal to %d", 2**idx_bits
        sys.exit()
    ...
    # clip to [0~15]
    # param_idx_low_bit = param_idx[np.arange(0, len(param_idx), 2)]* 2**wb_bits + param_idx[np.arange(1, len(param_idx), 2)]
