...
void read_half_layer(ifstream &in, int layer_id, vector<char> &min_exp_w,
  ...
  // conv layers' & squeeze 3x3 layers' weights, except for the last conv layer.
  // if ((layer_id - 1) % 3 == 0 && layer_id != 1 && PT == WEIGHT) {
  // if (((layer_id - 1) % 3 == 0 || layer_id == 26) && PT == WEIGHT) {
  if ((layer_id - 1) % 3 == 0 && PT == WEIGHT) {
    // read the short
  ...
  } else { // normal layer or squeeze1x1 layer or conv1x1 layer(i.e. last layer)
    // read the short
